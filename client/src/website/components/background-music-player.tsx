...
export interface MusicPlayerThemeTokens {
  primary?: string;
  accent?: string;
  iconColor?: string;
  glow?: string;
...
interface BackgroundMusicPlayerProps {
  ...
  /** Per-template colour tokens; falls back to amber/gold if omitted. */
  theme?: MusicPlayerThemeTokens;
}
...
export function BackgroundMusicPlayer({
  ...
  return (
    <div
      ...
      style={{ ['--bgmp-thumb' as any]: accent }}
    >
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-full shadow-lg border border-gray-200 p-2">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            variant="ghost"
            className={`w-8 h-8 p-0 rounded-full relative ${hasError
                ? 'hover:bg-red-100'
                : 'hover:bg-gray-100'
              }`}
          ...
          <div className="relative">
            <Music
              className={`w-4 h-4 ${hasError ? 'text-red-500' : ''} ${isReady && !isPlaying ? 'animate-pulse' : ''}`}
              style={
                hasError
                  ? undefined
                  : isPlaying
          ...
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              style={{
                background: `linear-gradient(to right, ${accent} 0%, ${accent} ${(isMuted ? 0 : volume) * 100}%, #e5e7eb ${(isMuted ? 0 : volume) * 100}%, #e5e7eb 100%)`
              }}
            />
          </div>
        </div>
...
style.textContent = `
  .slider::-webkit-slider-thumb {
    ...
    width: 12px;
    border-radius: 50%;
    background: var(--bgmp-thumb, #D4AF76);
    cursor: pointer;
...
`;
if (typeof document !== 'undefined' && !document.getElementById('bgmp-slider-style')) {
  style.id = 'bgmp-slider-style';
